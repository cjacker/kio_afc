...
AfcDevice::AfcDevice( const char* id, AfcProtocol* proto ) :_proto(proto), openFd(-1)
{
    ...
    ret = lockdownd_client_new_with_handshake (_dev, &control, "kio_afc");

    if( ret == LOCKDOWN_E_SUCCESS) {
        lockdownd_service_descriptor_t service;
        ret = lockdownd_start_service (control, AFC_SERVICE_NAME, &service);
        if( ret == LOCKDOWN_E_SUCCESS ) {
            afc_client_new (_dev, service, &_afc);
            //device name
            char* name = NULL;
            lockdownd_get_device_name (control, &name);
            _name = name;
            free (name);
               //device model
            if ( NULL != _afc )
            ...
            _proto->error(ERR_ACCESS_DENIED, QString::fromLatin1("Failed to connect to the device. Please check your device and try again."));
        }
    } else {
        _proto->error(ERR_ACCESS_DENIED, QString::fromLatin1("Failed to connect to the device. Please check your device and try again."));
    }
...
bool AfcDevice::checkError( afc_error_t error, KIO::Error& err_id )
{
    ...
    qCDebug(LOG_KIO_AFC) << "error: " << err_id << "ret: " << ret;
...
{
    qCDebug(LOG_KIO_AFC) << path << _flags;
    ...
    if ( !path.isEmpty() )
    {
        ...
        {
            qCDebug(LOG_KIO_AFC) << "Deleting destination file" << path;
            if ( ! del (path, error) )
                return false;
        }
    ...
    if (result < 0)
    {
        qCDebug(LOG_KIO_AFC) << "Error during 'put'. Aborting.";
...
{
    qCDebug(LOG_KIO_AFC) << path << "mode: " << mode;
